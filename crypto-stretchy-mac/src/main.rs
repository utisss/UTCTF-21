...
async fn program(addr: SocketAddr) {
    ...
    let minimum_key_size =
        env::get_var(MINIMUM_KEY_SIZE_VAR, DEFAULT_MINIMUM_KEY_SIZE).unwrap();
    let maximum_key_size =
        env::get_var(MAXIMUM_KEY_SIZE_VAR, DEFAULT_MAXIMUM_KEY_SIZE).unwrap();
